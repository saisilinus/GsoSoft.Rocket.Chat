...
const pickRandom = (arr: any[]): any => arr[Math.floor(Math.random() * arr.length)];
...
export const generateTasks = (
	...
	ModelOptionalId<ITask>,
	'_id' | 'title' | 'description' | 'startDate' | 'endDate' | 'assignedBy' | 'assignedTo' | 'type' | 'sortOrder' | 'status' | 'reward'
>[] => {
	const taskList: Omit<ITask, '_id' | '_updatedAt'>[] = [];
